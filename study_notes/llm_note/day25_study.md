...
## Goal
Build a concrete mental model of **Study RNN intuition and limitations** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study RNN intuition and limitations. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day25_study.py`, inspect every printed value, and change one input or constant.
3. Update a simple recurrent state over tokens and identify where old information can fade.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
hidden state, recurrence, vanishing gradients, sequential dependence.
