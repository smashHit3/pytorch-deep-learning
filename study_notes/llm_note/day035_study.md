...
## Goal
Build a concrete mental model of **Review all Transformer submodules in one diagram** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Review all Transformer submodules in one diagram. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day035_study.py`, inspect every printed value, and change one input or constant.
3. Label a compact Transformer block data-flow diagram from input IDs to logits.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
embedding, attention, residual path, feed-forward network, output head.
