...
## Goal
Build a concrete mental model of **Study residual connections and layer normalization** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study residual connections and layer normalization. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day034_study.py`, inspect every printed value, and change one input or constant.
3. Apply a residual update then normalization; compare statistics before and after.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
skip connections, normalization, stable optimization, pre-norm blocks.
