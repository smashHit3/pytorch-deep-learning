...
## Goal
Build a concrete mental model of **Study chunking, metadata, and context packing** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study chunking, metadata, and context packing. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day086_study.py`, inspect every printed value, and change one input or constant.
3. Chunk a fixed document with overlap and preserve page/source metadata.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
chunk size, overlap, metadata filters, token budget, packing.
