...
## Goal
Build a concrete mental model of **Learn next-token prediction and autoregressive modeling** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn next-token prediction and autoregressive modeling. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day043_study.py`, inspect every printed value, and change one input or constant.
3. Create shifted targets for a short token sequence and compute a next-token loss.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
shifted targets, conditional probability, teacher forcing, causal objective.
