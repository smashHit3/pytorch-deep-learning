...
## Goal
Build a concrete mental model of **Build a small prompt comparison set for one task** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Build a small prompt comparison set for one task. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day73_study.py`, inspect every printed value, and change one input or constant.
3. Create a small prompt comparison table with controls and expected answers.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
test set, prompt variants, controls, expected outputs.
