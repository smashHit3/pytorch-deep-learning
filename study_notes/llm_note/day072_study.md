...
## Goal
Build a concrete mental model of **Learn structured outputs and output constraints** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn structured outputs and output constraints. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day072_study.py`, inspect every printed value, and change one input or constant.
3. Validate a locally constructed structured response against required keys and types.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
schemas, validation, constrained fields, retries, parsing.
