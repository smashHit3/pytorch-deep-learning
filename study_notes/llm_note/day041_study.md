...
## Goal
Build a concrete mental model of **Trace tensor shapes through the full block** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Trace tensor shapes through the full block. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day041_study.py`, inspect every printed value, and change one input or constant.
3. Use a shape table to compute head dimension and verify every reshape.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
batch, sequence length, embedding width, heads, head dimension.
