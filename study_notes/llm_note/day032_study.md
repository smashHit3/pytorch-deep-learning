...
## Goal
Build a concrete mental model of **Study scaled dot-product attention step by step** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study scaled dot-product attention step by step. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day032_study.py`, inspect every printed value, and change one input or constant.
3. Repeat attention with and without scaling and compare softmax sharpness.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
dot products, scaling by sqrt(d_k), softmax, weighted values.
