...
## Goal
Build a concrete mental model of **Write a prompt engineering checklist for future projects** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Write a prompt engineering checklist for future projects. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day077_study.py`, inspect every printed value, and change one input or constant.
3. Generate and review a concise prompt checklist before a hypothetical model call.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
role, context, examples, constraints, format, evaluation.
