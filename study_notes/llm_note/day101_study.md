...
## Goal
Build a concrete mental model of **Learn quantization basics and memory tradeoffs** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn quantization basics and memory tradeoffs. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day101_study.py`, inspect every printed value, and change one input or constant.
3. Quantize a list of weights to a small integer range and report error/memory tradeoff.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
precision, quantization error, memory footprint, calibration.
