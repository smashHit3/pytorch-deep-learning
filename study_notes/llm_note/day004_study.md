...
## Goal
Build a concrete mental model of **Learn PyTorch tensors, dtype, device, and basic math operations** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn PyTorch tensors, dtype, device, and basic math operations. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day004_study.py`, inspect every printed value, and change one input or constant.
3. Create float and integer tensors, print their dtype/device, and compare elementwise and matrix operations.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
tensor dtype, device placement, elementwise arithmetic, reductions.
