...
## Goal
Build a concrete mental model of **Review the week and write short notes on tensors, gradients, and training loops** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Review the week and write short notes on tensors, gradients, and training loops. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day007_study.py`, inspect every printed value, and change one input or constant.
3. Re-run one prior tensor and one prior training example and record one shape and one gradient invariant.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
tensor shapes, gradients, loss, parameter updates.
