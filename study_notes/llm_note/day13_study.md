...
## Goal
Build a concrete mental model of **Compare SGD and Adam in a toy example** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Compare SGD and Adam in a toy example. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day13_study.py`, inspect every printed value, and change one input or constant.
3. Run the toy optimizer trajectories and compare the final parameter and loss.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
SGD momentum, Adam moments, adaptive steps, fair comparisons.
