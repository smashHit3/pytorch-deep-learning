...
## Goal
Build a concrete mental model of **Measure prompt quality using consistency and error analysis** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Measure prompt quality using consistency and error analysis. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day074_study.py`, inspect every printed value, and change one input or constant.
3. Aggregate repeated prompt outputs and assign each miss to an explicit error category.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
consistency, confusion matrix, error buckets, prompt comparison.
