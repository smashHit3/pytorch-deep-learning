...
## Goal
Build a concrete mental model of **Define scope, dataset, evaluation method, and demo goal** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Define scope, dataset, evaluation method, and demo goal. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day107_study.py`, inspect every printed value, and change one input or constant.
3. Write a machine-readable local project brief with scope, data, evaluation, and demo goal.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
scope boundary, local data, rubric, demo flow, risks.
