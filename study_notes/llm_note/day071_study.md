...
## Goal
Build a concrete mental model of **Study zero-shot, few-shot, and chain-of-thought prompting** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study zero-shot, few-shot, and chain-of-thought prompting. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day071_study.py`, inspect every printed value, and change one input or constant.
3. Render zero-shot and few-shot prompt variants for the same classification task.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
zero-shot, few-shot, rationale prompting, prompt sensitivity.
