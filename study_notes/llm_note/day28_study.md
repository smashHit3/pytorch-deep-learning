...
## Goal
Build a concrete mental model of **Write notes on why Transformers replaced recurrent models** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Write notes on why Transformers replaced recurrent models. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day28_study.py`, inspect every printed value, and change one input or constant.
3. Write two reasons attention improves on a recurrent bottleneck and one remaining limitation.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
sequential bottleneck, parallelism, long-range dependencies, attention.
