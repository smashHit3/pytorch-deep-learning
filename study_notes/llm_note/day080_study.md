...
## Goal
Build a concrete mental model of **Study cosine similarity and nearest-neighbor search** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study cosine similarity and nearest-neighbor search. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day080_study.py`, inspect every printed value, and change one input or constant.
3. Rank local documents by cosine similarity for a query.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
cosine similarity, normalization, ranking, nearest neighbors.
