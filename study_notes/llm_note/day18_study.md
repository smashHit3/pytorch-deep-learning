...
## Goal
Build a concrete mental model of **Train a small MLP on a simple dataset** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Train a small MLP on a simple dataset. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day18_study.py`, inspect every printed value, and change one input or constant.
3. Train a small MLP on a synthetic pattern and report loss and accuracy.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
train loop, minibatches, classification accuracy, loss curves.
