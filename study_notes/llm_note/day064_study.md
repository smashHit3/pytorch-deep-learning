...
## Goal
Build a concrete mental model of **Learn the difference between base models and instruction-tuned models** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn the difference between base models and instruction-tuned models. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day064_study.py`, inspect every printed value, and change one input or constant.
3. Format the same user request as base continuation and instruction-response examples.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
base completion behavior, instructions, alignment data, assistant format.
