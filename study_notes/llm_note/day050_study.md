...
## Goal
Build a concrete mental model of **Study a small implementation such as nanoGPT or minGPT** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study a small implementation such as nanoGPT or minGPT. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day050_study.py`, inspect every printed value, and change one input or constant.
3. Read the local mini-architecture sketch and annotate where masks, embeddings, and logits occur.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
module layout, data flow, causal mask, parameter count, reading code.
