...
## Goal
Build a concrete mental model of **Build the first end-to-end version** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Build the first end-to-end version. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day108_study.py`, inspect every printed value, and change one input or constant.
3. Run an end-to-end local input → retrieval/logic → output smoke test.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
input, processing, output, local end-to-end flow, smoke test.
