...
## Goal
Build a concrete mental model of **Study positional encoding and why order information is needed** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study positional encoding and why order information is needed. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day036_study.py`, inspect every printed value, and change one input or constant.
3. Add positional features to identical token embeddings and compare positions.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
position information, sinusoidal features, learned positional embeddings.
