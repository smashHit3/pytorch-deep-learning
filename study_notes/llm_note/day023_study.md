...
## Goal
Build a concrete mental model of **Study tokenization basics: word, subword, and character tokenization** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study tokenization basics: word, subword, and character tokenization. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day023_study.py`, inspect every printed value, and change one input or constant.
3. Tokenize one sentence by words and characters; list one tradeoff for each.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
word tokens, character tokens, subwords, vocabulary tradeoffs.
