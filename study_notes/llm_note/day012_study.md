...
## Goal
Build a concrete mental model of **Learn optimization concepts: learning rate, batch size, and epochs** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn optimization concepts: learning rate, batch size, and epochs. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day012_study.py`, inspect every printed value, and change one input or constant.
3. Train the same small model with two learning rates and explain the stability difference.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
learning rate, batch size, epoch, convergence, stability.
