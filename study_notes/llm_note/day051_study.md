...
## Goal
Build a concrete mental model of **Implement embeddings, positional embeddings, and input pipeline** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Implement embeddings, positional embeddings, and input pipeline. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day051_study.py`, inspect every printed value, and change one input or constant.
3. Construct token and positional embeddings, add them, and check the batch/sequence shape.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
token embeddings, positional embeddings, batching, shifted labels.
