...
## Goal
Build a concrete mental model of **Improve the retriever or prompt template in your RAG app** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Improve the retriever or prompt template in your RAG app. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day087_study.py`, inspect every printed value, and change one input or constant.
3. Compare baseline and improved retrieval/prompt settings on the same local query.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
query rewrite, ranking, prompt template, retrieval ablation.
