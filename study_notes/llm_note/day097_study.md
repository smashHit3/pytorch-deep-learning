...
## Goal
Build a concrete mental model of **Compare several outputs and rank them with clear criteria** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Compare several outputs and rank them with clear criteria. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day097_study.py`, inspect every printed value, and change one input or constant.
3. Aggregate pairwise rankings using clear criteria and expose ties.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
pairwise ranking, criteria, ties, aggregation, rationale.
