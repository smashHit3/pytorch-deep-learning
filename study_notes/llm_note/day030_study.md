...
## Goal
Build a concrete mental model of **Learn query, key, and value intuition with a tiny example** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn query, key, and value intuition with a tiny example. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day030_study.py`, inspect every printed value, and change one input or constant.
3. Calculate a query-key score and use it to weight two values.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
queries, keys, values, compatibility scores, weighted reads.
