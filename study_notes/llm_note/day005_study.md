...
## Goal
Build a concrete mental model of **Study autograd; compute gradients for a tiny linear model** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study autograd; compute gradients for a tiny linear model. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day005_study.py`, inspect every printed value, and change one input or constant.
3. Differentiate a scalar linear prediction and verify the reported gradient by hand.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
computational graphs, requires_grad, derivatives, gradient accumulation.
