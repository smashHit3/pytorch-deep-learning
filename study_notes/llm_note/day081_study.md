...
## Goal
Build a concrete mental model of **Learn vector database basics and retrieval workflow** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn vector database basics and retrieval workflow. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day081_study.py`, inspect every printed value, and change one input or constant.
3. Build an in-memory list-based vector index with metadata and query it.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
indexing, metadata, persistence, query/retrieve workflow.
