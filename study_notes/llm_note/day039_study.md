...
## Goal
Build a concrete mental model of **Study causal masking for autoregressive generation** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study causal masking for autoregressive generation. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day039_study.py`, inspect every printed value, and change one input or constant.
3. Verify that each position assigns zero probability to future positions.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
lower-triangular mask, future-token leakage, autoregressive factorization.
