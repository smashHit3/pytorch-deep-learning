...
## Goal
Build a concrete mental model of **Study hallucination patterns and when prompts fail** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study hallucination patterns and when prompts fail. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day076_study.py`, inspect every printed value, and change one input or constant.
3. Classify fixed failure examples as unsupported, ambiguous, or safely abstained.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
fabrication, ambiguity, missing context, abstention, verification.
