...
## Goal
Build a concrete mental model of **Learn regularization, dropout, and weight decay** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn regularization, dropout, and weight decay. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day020_study.py`, inspect every printed value, and change one input or constant.
3. Apply L2 penalty or dropout in a toy model and compare a parameter norm.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
L2 weight decay, dropout, regularization strength, bias-variance.
