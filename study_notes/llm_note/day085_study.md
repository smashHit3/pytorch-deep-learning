...
## Goal
Build a concrete mental model of **Learn the architecture of retrieval-augmented generation** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn the architecture of retrieval-augmented generation. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day085_study.py`, inspect every printed value, and change one input or constant.
3. Trace a local RAG request through retrieval, packing, grounded answer, and source list.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
retriever, context builder, generator, source attribution, fallback.
