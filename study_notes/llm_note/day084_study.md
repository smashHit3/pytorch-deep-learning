...
## Goal
Build a concrete mental model of **Evaluate retrieval quality and improve chunking strategy** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Evaluate retrieval quality and improve chunking strategy. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day084_study.py`, inspect every printed value, and change one input or constant.
3. Measure recall@k over labeled local queries and vary a chunk boundary.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
recall@k, chunk boundaries, relevance labels, failure analysis.
