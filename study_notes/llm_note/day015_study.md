...
## Goal
Build a concrete mental model of **Study linear regression and implement it in PyTorch** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study linear regression and implement it in PyTorch. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day015_study.py`, inspect every printed value, and change one input or constant.
3. Fit a linear relation, inspect learned parameters, and make one prediction.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
linear model, mean squared error, fitted slope and intercept.
