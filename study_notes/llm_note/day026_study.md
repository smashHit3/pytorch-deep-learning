...
## Goal
Build a concrete mental model of **Learn LSTM and GRU at a high level** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn LSTM and GRU at a high level. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day026_study.py`, inspect every printed value, and change one input or constant.
3. Compare a simple gate calculation with an ungated recurrent update.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
gates, memory cell, GRU update/reset gates, long dependencies.
