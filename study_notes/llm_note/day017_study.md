...
## Goal
Build a concrete mental model of **Learn multilayer perceptrons and activation functions** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn multilayer perceptrons and activation functions. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day017_study.py`, inspect every printed value, and change one input or constant.
3. Change an activation in a tiny MLP and compare its outputs on the same inputs.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
hidden layers, nonlinear activations, representation learning.
