...
## Goal
Build a concrete mental model of **Review vectors, matrices, dot products, and matrix multiplication** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Review vectors, matrices, dot products, and matrix multiplication. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day8_study.py`, inspect every printed value, and change one input or constant.
3. Compute a dot product and a matrix product by hand before checking the local tensor result.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
vectors, matrices, dot products, matrix multiplication, dimensions.
