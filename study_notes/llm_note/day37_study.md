...
## Goal
Build a concrete mental model of **Read the encoder-decoder overview in *Attention Is All You Need*** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Read the encoder-decoder overview in *Attention Is All You Need*. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day37_study.py`, inspect every printed value, and change one input or constant.
3. Trace a toy source sequence through encoder memory and a decoder query.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
encoder representations, decoder generation, cross-attention, teacher forcing.
