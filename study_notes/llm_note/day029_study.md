...
## Goal
Build a concrete mental model of **Study the problem attention is trying to solve** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study the problem attention is trying to solve. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day029_study.py`, inspect every printed value, and change one input or constant.
3. Compute attention weights over three context items and observe selective focus.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
fixed context bottleneck, selective context, weighted aggregation, alignment.
