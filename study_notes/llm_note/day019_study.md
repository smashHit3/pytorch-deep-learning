...
## Goal
Build a concrete mental model of **Study overfitting, validation, and train/test split** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study overfitting, validation, and train/test split. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day019_study.py`, inspect every printed value, and change one input or constant.
3. Make a deterministic split, compare train versus validation error, and identify potential overfitting.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
held-out validation, leakage, generalization gap, train/test split.
