...
## Goal
Build a concrete mental model of **Write notes on why loss goes down but generation can still be bad** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Write notes on why loss goes down but generation can still be bad. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day049_study.py`, inspect every printed value, and change one input or constant.
3. Pair a loss number with generated samples and list why neither alone proves quality.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
average loss, calibration, exposure bias, data limits, qualitative evaluation.
