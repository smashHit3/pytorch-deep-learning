...
## Goal
Build a concrete mental model of **Re-implement a Transformer block in PyTorch** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Re-implement a Transformer block in PyTorch. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day040_study.py`, inspect every printed value, and change one input or constant.
3. Run a local Transformer-block-shaped module and print input/output shapes.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
layer norm, masked attention, MLP, residual connections.
