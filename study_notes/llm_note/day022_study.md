...
## Goal
Build a concrete mental model of **Learn what makes text sequential data different from tabular data** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn what makes text sequential data different from tabular data. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day022_study.py`, inspect every printed value, and change one input or constant.
3. Turn a short sentence into shifted input-target sequence pairs and identify why order matters.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
order, context windows, variable length, next-item prediction.
