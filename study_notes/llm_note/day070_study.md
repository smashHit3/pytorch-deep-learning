...
## Goal
Build a concrete mental model of **Evaluate outputs and write down common failure cases** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Evaluate outputs and write down common failure cases. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day070_study.py`, inspect every printed value, and change one input or constant.
3. Run a rubric over fixed outputs and bucket errors instead of relying on a single score.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
test cases, rubric, failure taxonomy, regression checks.
