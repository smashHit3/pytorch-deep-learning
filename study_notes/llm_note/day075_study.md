...
## Goal
Build a concrete mental model of **Learn task metrics such as accuracy, F1, BLEU, or ROUGE** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn task metrics such as accuracy, F1, BLEU, or ROUGE. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day075_study.py`, inspect every printed value, and change one input or constant.
3. Compute accuracy, precision, recall, and F1 from a fixed confusion matrix.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
accuracy, precision, recall, F1, overlap metrics, metric limitations.
