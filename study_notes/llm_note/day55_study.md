...
## Goal
Build a concrete mental model of **Debug shape issues, masking, and sampling behavior** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Debug shape issues, masking, and sampling behavior. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day55_study.py`, inspect every printed value, and change one input or constant.
3. Trigger or inspect a shape/mask assertion, then state the expected invariant.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
assertions, tensor dimensions, mask direction, deterministic seeds.
