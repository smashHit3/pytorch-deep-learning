...
## Goal
Build a concrete mental model of **Summarize alignment versus capability in simple language** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Summarize alignment versus capability in simple language. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day098_study.py`, inspect every printed value, and change one input or constant.
3. Contrast a capable output with a safe/unsafe output using a short scenario.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
capability, alignment, incentives, safeguards, residual risk.
