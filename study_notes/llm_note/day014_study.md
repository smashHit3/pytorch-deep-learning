...
## Goal
Build a concrete mental model of **Summarize backpropagation and optimization in your own words** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Summarize backpropagation and optimization in your own words. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day014_study.py`, inspect every printed value, and change one input or constant.
3. Trace the chain rule through a two-operation scalar graph and state what flows backward.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
chain rule, local gradients, reverse-mode autodiff, optimization loop.
