...
## Goal
Build a concrete mental model of **Add attention and feed-forward blocks** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Add attention and feed-forward blocks. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day052_study.py`, inspect every printed value, and change one input or constant.
3. Compose attention and feed-forward transformations with residual paths in a tiny sketch.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
attention block, feed-forward block, residual path, logits.
