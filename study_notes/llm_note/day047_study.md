...
## Goal
Build a concrete mental model of **Learn greedy decoding, sampling, and temperature** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn greedy decoding, sampling, and temperature. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day047_study.py`, inspect every printed value, and change one input or constant.
3. Sample from fixed logits at two temperatures with a fixed seed.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
argmax, categorical sampling, temperature, reproducibility.
