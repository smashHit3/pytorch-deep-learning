...
## Goal
Build a concrete mental model of **Learn multi-head attention and why multiple heads help** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn multi-head attention and why multiple heads help. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day033_study.py`, inspect every printed value, and change one input or constant.
3. Split an embedding into heads and show how their outputs are concatenated.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
parallel heads, subspaces, concatenation, output projection.
