...
## Goal
Build a concrete mental model of **Prepare a tiny text dataset for language modeling** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Prepare a tiny text dataset for language modeling. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day44_study.py`, inspect every printed value, and change one input or constant.
3. Chunk a local string into overlapping input-target windows without downloading data.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
corpus cleaning, chunking, input-target pairs, batches.
