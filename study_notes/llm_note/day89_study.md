...
## Goal
Build a concrete mental model of **Add one tool or external function to your app** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Add one tool or external function to your app. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day89_study.py`, inspect every printed value, and change one input or constant.
3. Call a small allowlisted local calculator/search function and handle invalid input.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
local tool function, safe allowlist, inputs, outputs, error messages.
