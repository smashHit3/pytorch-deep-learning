...
## Goal
Build a concrete mental model of **Learn what embeddings are and how semantic similarity works** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn what embeddings are and how semantic similarity works. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day078_study.py`, inspect every printed value, and change one input or constant.
3. Map local texts to simple deterministic vectors and compare their similarity.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
vector representations, semantic neighborhoods, similarity signals.
