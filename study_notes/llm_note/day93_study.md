...
## Goal
Build a concrete mental model of **Study DPO at a conceptual level** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study DPO at a conceptual level. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day93_study.py`, inspect every printed value, and change one input or constant.
3. Compute a toy chosen-versus-rejected preference margin without presenting it as production DPO.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
chosen/rejected pairs, preference loss, reference policy, implicit reward.
