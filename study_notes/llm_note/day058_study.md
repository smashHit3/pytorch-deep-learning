...
## Goal
Build a concrete mental model of **Learn why dataset quality matters more than raw size in many cases** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn why dataset quality matters more than raw size in many cases. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day058_study.py`, inspect every printed value, and change one input or constant.
3. Score three local text records for duplication, relevance, and obvious noise.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
deduplication, relevance, noise, provenance, benchmark contamination.
