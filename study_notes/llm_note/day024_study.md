...
## Goal
Build a concrete mental model of **Learn embeddings and why token IDs need vector representations** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn embeddings and why token IDs need vector representations. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day024_study.py`, inspect every printed value, and change one input or constant.
3. Look up vectors for token IDs and compare two embedding rows.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
lookup tables, dense vectors, learned semantic features, embedding dimension.
