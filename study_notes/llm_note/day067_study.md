...
## Goal
Build a concrete mental model of **Study QLoRA and quantized fine-tuning concepts** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study QLoRA and quantized fine-tuning concepts. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day067_study.py`, inspect every printed value, and change one input or constant.
3. Quantize a small weight matrix locally and measure reconstruction error; do not claim QLoRA training.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
quantized base weights, adapters, memory savings, precision tradeoffs.
