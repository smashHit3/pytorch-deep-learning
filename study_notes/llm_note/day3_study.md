...
## Goal
Build a concrete mental model of **Study NumPy arrays and tensor shapes; practice reshape, transpose, slice, and broadcasting** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study NumPy arrays and tensor shapes; practice reshape, transpose, slice, and broadcasting. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day3_study.py`, inspect every printed value, and change one input or constant.
3. Represent a 2×3 nested list, reshape its flat values into 3×2, then explain which axis a transpose swaps.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
array shape, axes, reshape, transpose, slicing, broadcasting.
