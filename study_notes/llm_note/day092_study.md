...
## Goal
Build a concrete mental model of **Learn the goal of RLHF and preference optimization** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn the goal of RLHF and preference optimization. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day092_study.py`, inspect every printed value, and change one input or constant.
3. Rank fixed responses by a simple preference rubric and discuss the proxy limitation.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
preferences, reward models, policy optimization, helpfulness tradeoffs.
