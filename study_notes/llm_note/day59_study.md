...
## Goal
Build a concrete mental model of **Study scaling laws and model/data/compute tradeoffs** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study scaling laws and model/data/compute tradeoffs. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day59_study.py`, inspect every printed value, and change one input or constant.
3. Allocate a toy fixed budget across model size and data size and state the tradeoff.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
model size, token count, compute budget, diminishing returns.
