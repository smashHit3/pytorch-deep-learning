...
## Goal
Build a concrete mental model of **Learn probability basics: distributions, expectation, and variance** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn probability basics: distributions, expectation, and variance. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day010_study.py`, inspect every printed value, and change one input or constant.
3. Calculate empirical mean and variance for a small fixed sample and describe the distribution assumption.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
distributions, expectation, variance, empirical samples.
