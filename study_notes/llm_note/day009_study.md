...
## Goal
Build a concrete mental model of **Study derivatives, partial derivatives, and gradient descent intuition** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study derivatives, partial derivatives, and gradient descent intuition. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day009_study.py`, inspect every printed value, and change one input or constant.
3. Take one gradient-descent step on a quadratic and compare loss before and after.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
derivatives, partial derivatives, slope, gradient descent.
