...
## Goal
Build a concrete mental model of **Learn learning rate warmup, decay, and gradient clipping** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn learning rate warmup, decay, and gradient clipping. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day060_study.py`, inspect every printed value, and change one input or constant.
3. Print a warmup/decay schedule and clip an example gradient norm.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
warmup, cosine/linear decay, gradient norm, clipping.
