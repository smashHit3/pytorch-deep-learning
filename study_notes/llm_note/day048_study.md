...
## Goal
Build a concrete mental model of **Generate text and compare decoding strategies** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Generate text and compare decoding strategies. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day048_study.py`, inspect every printed value, and change one input or constant.
3. Generate several short continuations using greedy and sampled choices; compare diversity.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
decoding policy, randomness, diversity, repetition, seed control.
