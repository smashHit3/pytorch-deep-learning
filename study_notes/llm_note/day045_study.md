...
## Goal
Build a concrete mental model of **Build a vocabulary or simple tokenizer** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Build a vocabulary or simple tokenizer. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day045_study.py`, inspect every printed value, and change one input or constant.
3. Build an encode/decode vocabulary from a fixed corpus and test round-trip behavior.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
token-to-id mapping, unknown tokens, encode/decode, vocabulary coverage.
