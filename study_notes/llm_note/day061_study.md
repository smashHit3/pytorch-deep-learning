...
## Goal
Build a concrete mental model of **Study checkpoints, validation curves, and training instability** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study checkpoints, validation curves, and training instability. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day061_study.py`, inspect every printed value, and change one input or constant.
3. Save a local in-memory checkpoint dictionary and identify signals of instability.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
checkpoint state, validation curve, divergence, recovery.
