...
## Goal
Build a concrete mental model of **Learn feed-forward blocks and attention masks** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn feed-forward blocks and attention masks. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day038_study.py`, inspect every printed value, and change one input or constant.
3. Apply a mask to attention scores and inspect a token-wise feed-forward transform.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
position-wise MLP, padding mask, causal mask, token-wise transformation.
