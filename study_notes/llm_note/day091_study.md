...
## Goal
Build a concrete mental model of **Write notes on when to use plain prompting, RAG, or tools** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Write notes on when to use plain prompting, RAG, or tools. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day091_study.py`, inspect every printed value, and change one input or constant.
3. Use a decision table to choose prompt-only, retrieval, or a tool for fixed scenarios.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
knowledge in prompt, external knowledge, deterministic actions, costs.
