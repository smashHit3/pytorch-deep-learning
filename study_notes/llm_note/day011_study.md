...
## Goal
Build a concrete mental model of **Study softmax, cross-entropy, and why classification uses them** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Study softmax, cross-entropy, and why classification uses them. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day011_study.py`, inspect every printed value, and change one input or constant.
3. Convert logits to probabilities and manually identify the cross-entropy penalty for the correct class.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
logits, softmax probabilities, negative log likelihood, cross-entropy.
