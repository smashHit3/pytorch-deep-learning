...
## Goal
Build a concrete mental model of **Generate embeddings for a small document set** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Generate embeddings for a small document set. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day079_study.py`, inspect every printed value, and change one input or constant.
3. Embed a tiny fixed document collection with a dependency-free feature extractor.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
document chunks, local feature vectors, deterministic embedding stand-in.
