...
## Goal
Build a concrete mental model of **Learn the inference pipeline from tokens to generated text** and connect it to the larger LLM training or application pipeline.

## Today's Focus
Learn the inference pipeline from tokens to generated text. Use the small local exercise to make the idea observable before generalizing it to production-scale LLM work.
...
2. Run `python study_notes/llm_daily/day99_study.py`, inspect every printed value, and change one input or constant.
3. Trace a local token list through a deterministic logits-and-decoding simulation.
4. Record one observation, one limitation, and one question for the next study session.

## Key Concepts
tokenization, forward pass, logits, decoding, detokenization.
