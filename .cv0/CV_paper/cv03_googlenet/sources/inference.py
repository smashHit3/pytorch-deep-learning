...
def parse_args():
    parser = ArgumentParser(description="Run GoogleNet inference on a single image.")
    parser.add_argument(
        "--image",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        "--model-path",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "results" / "googlenet.pth",
        help="Path to the saved GoogleNet state dictionary.",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def load_model(model_path: Path, device: torch.device) -> torch.nn.Module:
    ...
    model = GoogleNet(num_classes=len(CLASS_NAMES))
