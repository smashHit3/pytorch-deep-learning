...
from tools.data_processor import load_data_fashion_mnist
from argparse import ArgumentParser
from googlenet import GoogleNet
import torch

def parse_args():
    parser = ArgumentParser(description="Train a GoogleNet model on the Fashion-MNIST dataset")
    ...
    parser.add_argument("--save-path", type=Path, default=Path(__file__).resolve().parents[1] / "results" / "googlenet.pth",
...
def main():
    args = parse_args()
    train_loader, val_loader = load_data_fashion_mnist(args.batch_size, resize=224)
    ...
    print(f"Saved GoogleNet weights to {args.save_path}")
